<?	
	...
	*************************************************************************************************************************/	
	...
	include_once(dirname(dirname(__FILE__)) . '/includes/penname.functions.php');
	include_once(dirname(dirname(__FILE__)) . '/includes/article.functions.php');
	include_once(dirname(dirname(__FILE__)) . '/includes/site.functions.php');
	include_once(dirname(dirname(__FILE__)) . '/includes/profile.functions.php');
	include_once(dirname(dirname(__FILE__)) . '/includes/mysql.functions.php');
	...
	function apply_action($_handler, &$_html, $_brand, $_tpl, $_db, &$_pagination) {					
		...
		if ( is_callable($_handler) && !empty($_handler) ) { 
			call_user_func_array( $_handler, array(&$_html, $_brand, $_tpl, $_db, $_pagination) );
		} else {
			call_user_func_array( "dashboard", array(&$_html, $_brand, $_tpl, $_db, $_pagination) );
		}		
	}
	...
	function highlight($words, $str) {
		...
		foreach($words as $word) {
			$len = strlen($word);
			if ($len >= 2) {
				foreach($str_words as $compare) {	
					$compare = preg_replace("/[^a-zA-Z0-9\']/i","",$compare);
					$_w = $stemmer->stem($word);
					$_c = $stemmer->stem($compare);
					if ($_c == $_w)
						$result = preg_replace("/\b(".preg_quote($compare).")\b/i","<b>$1</b>", $result);	
				}
			}
		}
	...
	function email($_email, $_name, $_message, $_subject) {
		...
		$_headers .= "From: iSnare.org Answers <noreply@isnare.org>\r\n";		
	...
	/***********************************************************************************
	* Please do not remove footer links to give attribution and support for ArticleFR+
	***********************************************************************************/
