<?
	...
	*************************************************************************************************************************/
	...
	function findPlugins($_connection) {
		$_plugindir = dirname(dirname(__FILE__)) . "/content/plugins/";
			...
			while ($entry = readdir($handle)) {
				if ($entry != '.' || $entry != '..') {
					...
					if (file_exists($_realpath . '/plugin.ini')) {
						...
						addPlugin($_realpath, $_ini["meta"]["name"], $_ini["meta"]["author"], $_ini["meta"]["site"], $_connection);
					}
				}
			}
		}
	...
	function addCategory($_category, $_connection) {
		if (!empty($_category)) {
			$_category = ucwords($_category);
			$_category = str_replace('And', 'and', $_category);
			$_q = "
				INSERT INTO category(name, parent)
					VALUES('" . mysqli_real_escape_string($_connection, $_category) . "', 0) ON DUPLICATE KEY UPDATE id = id";
			queryi($_q, $_connection);		
			return 1;
		} else {
			return 0;
	...
	function getSiteSettings($_connection) {
		...
		while( $_rs = mysqli_fetch_assoc($_result) ) {
			$_entry['name'] = $_rs['name'];
			$_entry['content'] = htmlspecialchars($_rs['content']);
			array_push($_retval, $_entry);
		}
